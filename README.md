...
- **AI 对话** — 接入 OpenAI 兼容 API（SiliconFlow、MiniMax 等），流式逐字打字机输出
- **思考过程可见** — 支持 `reasoning_content`（如 DeepSeek-R1），以暗色独立渲染
- **打字机特效** — 100ms tick 逐字缓冲输出，不卡不跳
- **鲨鱼游泳动画** — AI 回复时底部显示 `[▰ ▰ 🦈 ▰ ▰]` 动画，表示等待/生成中
- **背景图轮播** — 支持目录幻灯片，定时切换壁纸，可调亮度/透明度
- **鼠标滚轮** — 回复生成期间也可滚动查看历史
- **真彩色 + 同步输出** — VT 转义序列双缓冲差异刷新
...
AI 回复与横幅（logo）合并为一条可滚动的内容流。鼠标滚轮滚动：
- 每次滚动 3 行，AI 回复生成期间仍可滚动
- 底部分隔线以下的输入栏和工作目录行始终固定不动
- 发送新消息或收到流式输出时自动滚到底部
